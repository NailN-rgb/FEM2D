...
> bool AssembleHDG<IndexType, ValueType>::create_equation_system(const ell_equation_type &equation)
{
    try
    {
        ...
        for(std::size_t e = 0; e < N_triangles; e++)
        {
            ...
            // A = A_1 + A_2 + A_3 + E
            this->calculate_A1(equation, local_matrix, discrette_derivative, e);
...
)
{
    try
    {
        ...
        auto triangle_area = m_mesh->m_areas_triangle[element_index];
        auto l = m_mesh->get_l(element_index);
        ...
        // second column
        derivative_matrix(0, 1) = (triangle_points[2].y() - triangle_points[1].y()) / triangle_area;
        derivative_matrix(1, 1) = (triangle_points[1].x() - triangle_points[2].x()) / triangle_area;
        derivative_matrix(2, 1) = 24 / l;

        // third column
        derivative_matrix(0, 2) = (triangle_points[0].y() - triangle_points[2].y()) / triangle_area;
        derivative_matrix(1, 2) = (triangle_points[2].x() - triangle_points[0].x()) / triangle_area;
        derivative_matrix(2, 2) = 24 / l;

        // fourth column
        derivative_matrix(0, 3) = (triangle_points[1].y() - triangle_points[0].y()) / triangle_area;
        derivative_matrix(1, 3) = (triangle_points[0].x() - triangle_points[1].x()) / triangle_area;
...
)
{
    try
    {
        auto triangle_edge_connectivity = m_mesh->get_triangle_edges_id(element_index);
...
> bool AssembleHDG<IndexType, ValueType>::assemble_boundary_conditions(const ell_equation_type &ell_equation)
{
    try
    {
            ...
            [&](const auto& marker)
            {
                ...
                else
                {
                    throw std::runtime_error(
                        "Unknown type of BC at " 
                        + std::to_string(edge_idx) + "edge");
                }
        ...
        assemble_first_bc(first_bc_edges, ell_equation);
    }
    catch(const std::exception& e)
    {
