...
template<
    typename IndexType,
    typename ValueType
> auto Mesh_builder<IndexType, ValueType>::build_mesh(
    bool form_vtk
) -> Mesh_builder<IndexType, ValueType>::mesh_type_pointer
{
    this->parse();
    this->build_mesh();
...
> bool Mesh_builder<IndexType, ValueType>::build_mesh()
{
    try
    {
        ...
        // FIXME: programm will fall if we dont have 'e' key
        std::string triangle_switches("-pe");

        std::stringstream poly_file;

        create_poly_file(poly_file);
...
template<
    typename IndexType,
    typename ValueType
> auto Mesh_builder<IndexType, ValueType>::save_mesh(
    bool form_vtk
...
> bool Mesh_builder<IndexType, ValueType>::write_mesh_to_vtk()
{
    try
    {
        ...
        auto mesh_points = m_triangle_mesh->get_points_list(m_triangle_mesh->in_);
        auto mesh_triangles = m_triangle_mesh->get_triangle_list(m_triangle_mesh->in_); 
